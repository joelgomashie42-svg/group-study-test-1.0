...
import { GraduationCap, Trophy, CheckCircle2, XCircle, Lightbulb, ArrowLeft, RotateCcw, BookOpen, Search, Loader2, UserRound } from 'lucide-react';
...
export default function ResultsPage() {
  ...
  const findResults = async (event: FormEvent) => {
    event.preventDefault();
    const name = lookupName.trim();
    if (name.length < 2) {
      setLookupError('Please enter at least 2 characters of the name.');
      return;
    }
    ...
    try {
      ...
      if (!res.ok) throw new Error(payload.error || 'Could not find results.');
      if (!payload.results?.length) {
        setHistory([]);
        setData(null);
        setLookupError(`No saved results were found for “${name}”.`);
        return;
      }

      setHistory(payload.results);
      const latest = payload.results[0];
      setData({ ...latest.result_data, candidate_name: latest.candidate_name });
      sessionStorage.setItem('examResult', JSON.stringify({ ...latest.result_data, candidate_name: latest.candidate_name }));
    } catch (error: any) {
      setLookupError(error?.message || 'Could not retrieve results.');
    } finally {
      setSearching(false);
    }
  };
      ...
      <div className="min-h-screen bg-gradient-to-b from-amber-50 via-white to-white dark:from-slate-950 dark:via-slate-900 dark:to-slate-900">
        <header className="border-b border-border/40 backdrop-blur-sm bg-background/70 sticky top-0 z-50">
          <div className="max-w-4xl mx-auto px-6 h-16 flex items-center justify-between">
            ...
            <nav className="flex items-center gap-1">
              <Link href="/admin" className="px-3 py-2 text-sm font-medium text-muted-foreground hover:text-foreground">Admin</Link>
              <Link href="/take" className="px-3 py-2 text-sm font-medium text-muted-foreground hover:text-foreground">Take Test</Link>
        ...
        <div className="max-w-xl mx-auto px-6 py-24">
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-bold mb-2">Find Your Results</h1>
            <p className="text-muted-foreground">Enter the same name you used when taking the test.</p>
          </div>

          <Card className="p-6">
            <form onSubmit={findResults} className="space-y-4">
              <div className="space-y-2">
                <label htmlFor="result-name" className="text-sm font-semibold">Student Name</label>
                <div className="relative">
                  <UserRound className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
                  <input id="result-name" value={lookupName} onChange={(e) => setLookupName(e.target.value)} placeholder="e.g. Joel Gomashie" className="flex h-11 w-full rounded-md border border-input bg-background pl-10 pr-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" />
                </div>
              </div>
              {lookupError && <p className="text-sm text-destructive">{lookupError}</p>}
              <Button type="submit" size="lg" className="w-full" disabled={searching}>
                {searching ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" /> Searching...</> : <><Search className="w-4 h-4 mr-2" /> Find Results</>}
              </Button>
            </form>
          </Card>

          <div className="text-center mt-6">
            <Link href="/take"><Button variant="outline"><RotateCcw className="w-4 h-4 mr-2" /> Take a Test</Button></Link>
          </div>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-gradient-to-b from-amber-50 via-white to-white dark:from-slate-950 dark:via-slate-900 dark:to-slate-900">
      ...
      <div className="max-w-3xl mx-auto px-6 py-12">
        ...
        <Card className="p-5 mb-8">
          <form onSubmit={findResults} className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              <UserRound className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
              <input
                value={lookupName}
                onChange={(e) => setLookupName(e.target.value)}
                placeholder="Enter a name to find saved results"
                className="flex h-11 w-full rounded-md border border-input bg-background pl-10 pr-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
              />
            </div>
            <Button type="submit" disabled={searching}>
              {searching ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <Search className="w-4 h-4 mr-2" />}
              {searching ? 'Searching...' : 'Find Results'}
            </Button>
          </form>
          {lookupError && <p className="text-sm text-destructive mt-3">{lookupError}</p>}
        </Card>
          ...
          <div className="relative">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                {data.candidate_name && <p className="text-sm font-semibold text-foreground mt-1">Student: {data.candidate_name}</p>}
              </div>
            </div>
        ...
        {history.length > 1 && (
          <Card className="p-6 mb-8">
            <h2 className="text-lg font-bold mb-4">Your Previous Results</h2>
            <div className="space-y-2">
              {history.map((item) => (
                <button
                  key={item.id}
                  type="button"
                  onClick={() => setData({ ...item.result_data, candidate_name: item.candidate_name })}
                  className="w-full text-left flex items-center justify-between gap-4 p-3 rounded-lg border hover:bg-secondary/50 transition-colors"
                >
                  <span>
                    <span className="font-medium">{item.subject}</span>
                    <span className="block text-xs text-muted-foreground">{new Date(item.created_at).toLocaleString()}</span>
                  </span>
                  <span className="font-bold">{item.total_score}/{item.total_max}</span>
                </button>
              ))}
            </div>
          </Card>
